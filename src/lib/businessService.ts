...
export const businessService = {
  // Business Profile
  subscribeToProfile: (userId: string, callback: (profile: BusinessProfile | null) => void) => {
    const q = query(collection(db, 'profiles'), where('userId', '==', userId));
    return onSnapshot(q, (snapshot) => {
      if (snapshot.empty) {
        callback(null);
      } else {
        const profile = { id: snapshot.docs[0].id, ...snapshot.docs[0].data() } as BusinessProfile;
        callback(profile);
      }
    }, (error) => handleFirestoreError(error, OperationType.LIST, 'profiles'));
  },

  // Re-implementing properly for the service pattern:
  upsertProfile: async (profile: Omit<BusinessProfile, 'id'>) => {
    if (!auth.currentUser) throw new Error('User not authenticated');
    try {
      // Use userId as document ID for profiles to make it 1:1 and easy to upsert
